    ...
    case $1 in
        ...
        (-o|--output)       # Takes an option argument; ensure it has been specified.
            if [ "$2" ]; then
                ...
                die 'ERROR: "-o" or "--output" requires a non-empty option argument.'
            fi
            shift
            ;;
        (--output=?*)
...
joinStem () {
  ...
  ffmpeg -f concat -safe 0 -i concat-orig.txt -c copy "$OUTPUT_DIR"/"$NAME"/$STEM$SPLITS.$LOCAL_EXT
...
joinAllStems () {
  ...
  fileArray=( "${fileArray[@]/#/"$OUTPUT_DIR"/}" )
